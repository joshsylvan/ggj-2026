...
const MacBuzzHID = require('./mac-buzz-hid');
...
function setLeds(led1, led2, led3, led4) {
  if (!isBuzzConnected || !buzzers) return;
  buzzers.setLeds(led1, led2, led3, led4);
}

if (isMac) {
  try {
    buzzers = new MacBuzzHID();
    isBuzzConnected = buzzers.init();
    if (isBuzzConnected) {
      console.log('Using macOS HID buzz controller');
    }
  } catch (error) {
...
app.whenReady().then(() => {
  ipcMain.handle('setLED', (_, led1, led2, led3, led4) => {
...
app.on('before-quit', () => {
  if (isBuzzConnected && buzzers) {
    setLeds(false, false, false, false);

    // Clean up the HID device on macOS
    if (isMac && buzzers.close) {
      buzzers.close();
    }
  }
});
